...
class _MapCardState extends State<MapCard> {
  ...
  List<RequestsM>  tempStatusRequest = (await ApiHandler().getRequests())! ;
  
      for(int x = 0; x < tempStatusRequest.length; x++){
        if(tempStatusRequest[x].requestStatus == "PENDING"){
        
  var request = await http.get(Uri.parse(tempStatusRequest[x].vehiclesDetail[0].image));
...
Uint8List resizedImg = Uint8List.fromList(IMG.encodePng(resized));
 Marker resultMarker = Marker(
           icon:  BitmapDescriptor.fromBytes(resizedImg,size: Size.fromRadius(30)),
      ...
      snippet: 'Car Marker',
            ...
            position:
                LatLng(tempStatusRequest[x].serviceLocation.latitude,tempStatusRequest[x].serviceLocation.longitude),
                
            onTap: () {
                         ApiHandler().jobDetailed(tempStatusRequest[x].id, context,tempStatusRequest[x]);
