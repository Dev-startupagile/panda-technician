...
updateRquestCode(String a , String b , String c , String d){
  ...
  Container(
          ...
          const Text("Please enter verification Code",textAlign: TextAlign.center, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),)
,
const SizedBox(
  height: 20,
),
...
var response = await ApiHandler().verifyAccount(profile.id,a.text + b.text + c.text + d.text);
            if(response){
   Navigator.popAndPushNamed(context, "Profile");
            }else{
                showPurchaseDialog(context,"Error Occured","Wrong OTP");
            }
         ...
         onPressed: () async{
          Loading(context);
           ...
           var response = await ApiHandler().sendOtp(prefs.getString("userEmail").toString(),context);

           if(response){
             
                    Navigator.pushNamed(context, "Signup");

                                 showPurchaseDialog(context,"Otp Sent","Check Your Email",isOtp: true , isApiCall: false);




           }else{
                  showPurchaseDialog(context,"Error Occured","Wait for the count down to finsh");

           }
   
          },
          child:const Text('Resend OTP', style: TextStyle(fontSize: 14,color: Colors.white), textAlign: TextAlign.center,),
)  
            ),
  ],),
