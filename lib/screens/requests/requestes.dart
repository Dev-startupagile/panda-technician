...
class _RequestsState extends State<Requests> {
  ...
  late RequestsModel reque = RequestsModel(
    customerAvater:
  ...
  // late List<RequestsModel>? _requestModel = [];
  late List<RequestsM> _requestModel = [] as List<RequestsM>;

  void _getRequests() async {
    loading = true;
    _requestModel = (await ApiHandler().getRequests())!;
    canceld = (await ApiHandler().getCanceldRequest())!;
    print("DDDDD: " + canceld[0].requestId);
    loading = false;
    Future.delayed(const Duration(seconds: 1)).then((value) => setState(() {}));
  }
                                          ...
                                          return (_selectedIndex == 0)
                                              ? ((statusRequest[index].requestStatus == "ACCEPTED" ||
                                                      ...
                                                      (statusRequest[index].offerDetail.length > 0
                                                          ? !statusRequest[index]
                                                              .offerDetail[statusRequest[index]
                                                                      .offerDetail
                                                                      .length -
                                                                  1]
                                                              .isRejected
