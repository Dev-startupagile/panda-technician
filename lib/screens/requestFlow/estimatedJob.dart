...
void initState() {
 ...
 service = widget.arguments.service;
      ...
      statusIndex = widget.arguments.request.requestStatus == "ACCEPTED"? 0 : widget.arguments.request.requestStatus == "ON_MY_WAY"? 1 : widget.arguments.request.requestStatus == "ARRIVED" ? 2 : widget.arguments.request.requestStatus == "COMPLETED"? 3 :0;
...
void finshed(){
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar( 
        ...
        centerTitle: true,
        title: Text("Job in Progress",textAlign: TextAlign.center,
        ...
        children: <Widget>[
           Container(
              ...
              child: Row( mainAxisAlignment: MainAxisAlignment.spaceBetween, children:  <Widget>[
                Text(service.description, style: TextStyle(fontSize: 17,fontWeight: FontWeight.bold),textAlign: TextAlign.left,),
              
                Text(request.schedule.time, style: TextStyle(fontSize: 14, color: Colors.grey),textAlign: TextAlign.right)
  ...
  Icon(Icons.circle,color: statusIndex == 0? Colors.pink[100] : statusIndex == 1? Colors.blue[100] : statusIndex == 2? Colors.green[100] : Colors.grey[100] ,size:15),
              ...
              ,
                 ...
                 Text(request.schedule.date, style: TextStyle(fontSize: 14, color: Colors.grey),textAlign: TextAlign.right)

              ],  )
             ,),
             
             ((statusIndex != 4)?
     ProgressTracker(statusIndex: statusIndex, arrived: arrived, finshed: finshed,requestId: request.id,OnMyWay: ((){
       setState((){
         statusIndex = 1;
       });
     }),)
          ...
          child: Text("\$ 4500",style: TextStyle(height:1.5, fontSize: 14)),
          )
          ...
          child: 
          Padding(padding: EdgeInsets.all(10),
          child: Text("Service completed successfully",style: TextStyle(height:1.5, fontSize: 14)),
          )
          ...
          ,
          RatingCard(profile: request.customerInfo, vehicleImage: vehicle.image,),
     ...
     Container(
          ...
          child: 
          Padding(padding: EdgeInsets.fromLTRB(10, 10, 10, 0),
          child: Text("Changing Battery",style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ),
                  ),
                  Align(
          ...
          child: 
          Padding(padding: EdgeInsets.all(10),
          child: Text("The note created in the estimations is writing under here",style: TextStyle(height:1.5, fontSize: 14)),
          )
   ...
   updateStatus();
          ...
          child: const Text("View Estimation", style: TextStyle(fontSize: 14,color: Colors.white), textAlign: TextAlign.center,),
)  
            ),
       ...
       Container(  
         ...
         onPressed: () {
            ...
            Navigator.pushNamed(context, "CreateOffer",arguments: widget.arguments );
  ...
  // );
          ...
          child:const Text('CREATE ESTIMATE', style: TextStyle(fontSize: 14,color: Colors.white), textAlign: TextAlign.center,),
)  
            )
         :
         Container(
          ...
          child: Row( children: <Widget>[
              ...
              child: Column(children: <Widget>[
               ...
               Align(
                  alignment: Alignment.topLeft,
                  child: Text(service.description, style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
                 )
                  ,
                   Align(
                  alignment: Alignment.topLeft,
                  child:  Text( service.title, style: TextStyle(fontSize: 14, color: Colors.grey[400])),
                   )
        ...
        Container(
         ...
         onPressed: () {
              navigateTo(request.serviceLocation.latitude,request.serviceLocation.longitude);

          },
          child:Row(children: [
            
            Icon(Icons.location_on_outlined,color: Colors.grey[400]),
            Text("Find the best Route", style: TextStyle(color: Colors.grey[400]) )
          ]),
)  
           ,
        ),
  ...
  decoration:  BoxDecoration(
    ...
    color: Colors.transparent,
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.start, children: <Widget>[
                 Align(
                  alignment: Alignment.topLeft,
                  child:   Text(vehicle.brand, style: TextStyle(fontSize: 14,fontWeight: FontWeight.bold),textAlign: TextAlign.start ,)
                )
                ,
                Align(
                  
                  alignment: Alignment.topLeft,
                  child: Text(vehicle.model, style: TextStyle(fontSize: 14, color: Colors.grey[400]),textAlign: TextAlign.start,),
                ),
                  ...
                  Container(
                    ...
                    width:20,
                    height:20,
                    decoration: BoxDecoration(color: Color(int.parse('FF' + vehicle.color.replaceAll("#",""),radix: 16)), borderRadius: BorderRadius.circular(2)),
                  )
     ...
     const   Align(
          ...
          child: 
          Padding(padding: EdgeInsets.all(10),
          child: Text(request.description.note,style: TextStyle(height:1.5, fontSize: 14)),
          )
        ...
        width: MediaQuery.of(context).size.width * 0.9,
        height: 100,
        child: (request.description.attachments.length == 0)?

        CentredMessage(messageWidget:
         const Text("No Attachments Available",style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold)))
         
        : ListView.builder(
          scrollDirection: Axis.horizontal,
            itemCount: request.description.attachments.length,
            itemBuilder: (
            context,index){
               return  CurvedImage(callback: (){}, imageUrl: request.description.attachments[index]);
          }),
        )
         ...
         (request.requestStatus == "ACCEPTED" || request.requestStatus == "ARRIVED" || request.requestStatus == "ON_MY_WAY" )?
  Container(  
              width:MediaQuery.of(context).size.width * 0.9,//
              height:50,
              decoration:  BoxDecoration(borderRadius: BorderRadius.circular(10), color: Colors.grey.shade700),
   ...
   updateStatus();
          ...
          child: Text("Reschedule", style: const TextStyle(fontSize: 14,color: Colors.white), textAlign: TextAlign.center,),
)  
            ) : Container(  
              width:MediaQuery.of(context).size.width * 0.9,//
              height:50,
   ...
   updateStatus();
          ...
          child: Text("AcceptJob", style: const TextStyle(fontSize: 14,color: Colors.white), textAlign: TextAlign.center,),
)  
            ) ,
