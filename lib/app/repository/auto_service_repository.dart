...
class AutoServiceRepository {
  ...
  Future<List<ServiceRequestModel>> getTechnicianRequests() async {
    ...
    List<ServiceRequestModel> _model =
        List.from(response.data).map<ServiceRequestModel>((e) {
  ...
  Stream<gql.GraphQLResponse<dynamic>> onUpdateServiceRequest(
      String technicianId) {
    String graphQLDocument = '''
    subscription onUpdateServiceRequest(\$technicianId: String!) {
      onUpdateServiceRequest(technicianId: \$technicianId) {
        id
        customerId
        technicianId
        requestStatus
